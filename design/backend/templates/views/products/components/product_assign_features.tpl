{foreach from=$product_features item=feature key="feature_id"}
    ...
    {if $feature.feature_type != "ProductFeatures::GROUP"|enum}
        <div class="control-group">
            ...
            {if $feature.feature_type == "ProductFeatures::TEXT_SELECTBOX"|enum || $feature.feature_type == "ProductFeatures::NUMBER_SELECTBOX"|enum || $feature.feature_type == "ProductFeatures::EXTENDED"|enum}
                ...
                {if $feature.use_variant_picker}
                    ...
                    <input type="text" class="hidden-input{if $feature.feature_type == "ProductFeatures::NUMBER_SELECTBOX"|enum} cm-value-decimal{/if}" name="product_data[add_new_variant][{$feature.feature_id}][variant]" id="input_{$feature_id}" placeholder="{__("enter_other")}" />
                {else}
