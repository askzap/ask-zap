        ...
        <div class="pull-right hidden-tools">
            {capture name="items_tools"}
                ...
                {if !$non_editable && !$skip_delete}
                    <li>{btn type="text" text=__("delete") href={"products.delete_file?file_id=`$product_file.file_id`&product_id=`$product_data.product_id`"|fn_url} class="cm-confirm cm-tooltip cm-ajax cm-post cm-ajax-force cm-ajax-full-render cm-delete-row" data=["data-ca-target-id" => "product_files_list"]}</li>
                {/if}
            {/capture}
