...
<div class="step-container{if $edit}-active{/if}" data-ct-checkout="shipping_options" id="step_three">
    <h2 class="step-title{if $edit}-active{/if}{if !$show_number_of_steps}-complete{/if} clearfix">
        ...
        <a class="title{if !$show_number_of_steps} cm-ajax{/if}" {if $show_number_of_steps}href="{"checkout.checkout?edit_step=step_three&from_step=`$cart.edit_step`"|fn_url}" data-ca-target-id="checkout_*"{/if}>{__("shipping_options")}</a>
        {/hook}
    </h2>
