    ...
    foreach ($combination as $option_id => $variant_id) {
        switch ($variant_id) {
            case '-1':
                ...
                $result[] = $product_options[$option_id]['option_name'] . '=#Any';
                break;
            case '-2':
                // No variants can be selected
                $result[] = $product_options[$option_id]['option_name'] . '=#No';
                break;
            default:
                // Specified variant
                $result[] = $product_options[$option_id]['option_name'] . '=' . $product_options[$option_id]['variants'][$variant_id]['variant_name'];
                break;
        }
    }

    $result = implode($set_delimiter, $result);
