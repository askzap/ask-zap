...
if ($mode == 'view' && !empty($_REQUEST['product_id'])) {
    ...
    if ($data['age_verification'] == 'Y') {
        ...
        if (!$age) {
            ...
            Tygh::$app['view']->assign('content_tpl', 'addons/age_verification/views/products/components/form.tpl');

            return array (CONTROLLER_STATUS_OK);
        } else {
            if ($age < $data['age_limit']) {
                fn_set_notification('E', __('error'), __('access_denied'));
                Tygh::$app['view']->assign('content_tpl', 'addons/age_verification/views/products/components/deny.tpl');
