    ...
    (function($) {
        ...
        function saveFormValues(form) {
            form.find(inputs_selector).each(function(){
                var elm = $(this);
                form_values[elm.attr('name') + '_' + elm.attr('id')] = elm.val();
            });
        }
        ...
        function checkFormChanged(form) {
            ...
            form.find(inputs_selector).each(function(){
                var elm = $(this),
                    type = elm.attr('type');
                if (type != 'hidden' && form_values[elm.attr('name') + '_' + elm.attr('id')] != elm.val()) {
                    result = true;
                }
            });
    ...
    $(_.doc).on('click', '.cm-checkout-place-order', function() {
        ...
        $.ceEvent('on', 'ce.formpost_' + $(form).attr('name'), function() {
            $.toggleStatusBox('show', {
                content: '<span class="ty-ajax-loading-box-with__text-wrapper">' + _.tr('placing_order') + '</span>',
                class: 'ty-ajax-loading-box_text_block'
            });
        });
    });
